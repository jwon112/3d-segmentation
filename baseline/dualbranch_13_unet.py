...
class DualBranchUNet3D_MViT_Extended_Small(nn.Module):
    """Dual-branch UNet with MobileViT extended to FLAIR branch Stage 3,4 + MViT Stage5 (Small).
    ...
    def __init__(self, n_channels: int = 2, n_classes: int = 4, norm: str = 'bn', bilinear: bool = False, expand_ratio: float = 4.0):
        ...
        self.branch_flair3 = Down3DStrideMViT(32, 64, norm=self.norm, num_heads=4, mlp_ratio=2)
        ...
        self.branch_flair4 = Down3DStrideMViT(64, 128, norm=self.norm, num_heads=4, mlp_ratio=2)
...
class DualBranchUNet3D_MViT_Extended_Medium(nn.Module):
    """Dual-branch UNet with MobileViT extended to FLAIR branch Stage 3,4 + MViT Stage5 (Medium).
    ...
    def __init__(self, n_channels: int = 2, n_classes: int = 4, norm: str = 'bn', bilinear: bool = False, expand_ratio: float = 4.0):
        ...
        # Stage 3 branches: FLAIR = MobileViT, t1ce = MobileNetV2
        self.branch_flair3 = Down3DStrideMViT(64, 128, norm=self.norm, num_heads=4, mlp_ratio=2)
        self.branch_t1ce3 = Down3DMobileNetV2(64, 128, norm=self.norm, expand_ratio=self.expand_ratio)

        # Stage 4 branches: FLAIR = MobileViT, t1ce = MobileNetV2
        self.branch_flair4 = Down3DStrideMViT(128, 256, norm=self.norm, num_heads=4, mlp_ratio=2)
...
__all__ = [
    'DualBranchUNet3D_MViT_Extended_Small',
    'DualBranchUNet3D_MViT_Extended_Medium',
]
