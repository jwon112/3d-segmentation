...
class ChannelAdaptor(nn.Module):
    """Adapt input channels to match model expectations"""
...
class MobileUNETRWithChannelAdapter(nn.Module):
    """MobileUNETR with channel adapter at the top level"""
...
def MobileUNETR(img_size=(512, 512), patch_size=(16, 16), in_channels=2, out_channels=4):
    ...
    Args:
        img_size: Input image size (2D: (H, W), 3D: (D, H, W))
    ...
    if len(img_size) == 2:
        ...
        if in_channels == 2:
            ...
            wrapped_model = MobileUNETRWithChannelAdapter(model)
            return wrapped_model
        return model
    elif len(img_size) == 3:
        # 3D case (will be converted to 2D slices)
        model = build_mobileunetr_s(num_classes=out_channels, image_size=img_size[0])
        if in_channels == 2:
            wrapped_model = MobileUNETRWithChannelAdapter(model)
            return wrapped_model
        ...
        raise ValueError(f"Unsupported img_size dimension: {len(img_size)}")
