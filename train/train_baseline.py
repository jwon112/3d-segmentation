#!/usr/bin/env python3
"""
3D Segmentation Baseline Training Script
다중 모델, 다중 시드 실험을 위한 통합 훈련 스크립트
"""
...
def run_baseline_experiment(data_path, epochs=10, batch_size=1, seeds=[24], models=None):
    """3D Segmentation Baseline 실험 실행"""
    
    # 실험 결과 저장 디렉토리
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = f"baseline_results/baseline_experiment_results_{timestamp}"
    ...
    for seed in seeds:
        print(f"\n{'='*60}")
        print(f"Training Baseline Models with seed: {seed}")
        print(f"{'='*60}")
    ...
    # 결과 출력
    print("\n" + "="*80)
    print("3D SEGMENTATION BASELINE EXPERIMENT RESULTS SUMMARY")
    print("="*80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='3D Segmentation Baseline Experiment')
    ...
    print("Starting 3D Segmentation Baseline Experiment")
